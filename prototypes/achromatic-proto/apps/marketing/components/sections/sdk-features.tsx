  ...
  return (
    <div className="mt-4 h-[80px] rounded-xl bg-gradient-to-br from-muted/50 to-muted/30 border border-border/50 p-3 relative overflow-hidden">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[7px] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-500 ml-auto">
          &lt;50ms
        </span>
      </div>
      <svg
      ...
      >
        ...
        {nodes.map((node, i) => (
          <motion.circle
            ...
            r="4"
            fill={
              i === activeNode ? 'hsl(var(--primary))' : 'hsl(var(--muted))'
            }
            animate={
              shouldReduceMotion
                ? {}
...
function SyncMagic() {
  ...
  React.useEffect(() => {
    ...
    const states: Array<'ios' | 'android' | 'web' | 'synced'> = [
      'ios',
      'android',
      'web',
      'synced'
        ...
        {devices.map((device, i) => (
          <React.Fragment key={device.id}>
            <motion.div
              animate={{
                scale:
                  syncState === device.id || syncState === 'synced' ? 1.1 : 1,
                opacity:
                  syncState === device.id || syncState === 'synced' ? 1 : 0.5
              }}
...
function CodeTypeMagic() {
  const shouldReduceMotion = useReducedMotion();
  const codeLines = ['Adapty.activate()', 'makePurchase()', 'getPaywall()'];
...
const FEATURES = [
  {
    ...
    description:
      'Wherever your user is in the world, Adapty API will work blazingly fast, minimizing friction with payments.',
    magic: CDNMagic
  },
  {
    icon: RefreshCwIcon,
          ...
          <div className="mt-24">
            <div className="text-center mb-10">
              ...
              <p className="text-muted-foreground max-w-2xl mx-auto leading-relaxed">
                Get started with in-app payments in a couple of hours, even if
                you do it from scratch. You only need 3 SDK methods to begin.
              </p>
              <Link
