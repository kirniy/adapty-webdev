        ...
        <div key={ci} className="flex flex-col gap-0.5">
          {cohort.map((value, vi) => (
            <motion.div
              ...
              animate={shouldReduceMotion ? { opacity: value / 100, scale: 1 } : {
  ...
  return (
    <div className="mt-4 h-[60px] rounded-lg bg-muted/30 border border-border/50 p-2 flex items-center gap-3">
      ...
      <div className="flex gap-1">
        <motion.div
          animate={{
            backgroundColor: mode === 'days' ? 'hsl(var(--primary))' : 'hsl(var(--muted))',
            color: mode === 'days' ? 'hsl(var(--primary-foreground))' : 'hsl(var(--muted-foreground))',
          }}
        ...
        <motion.div
          animate={{
            backgroundColor: mode === 'renewals' ? 'hsl(var(--primary))' : 'hsl(var(--muted))',
            color: mode === 'renewals' ? 'hsl(var(--primary-foreground))' : 'hsl(var(--muted-foreground))',
          }}
          transition={{ duration: 0.2 }}
          className="px-2 py-1 rounded text-[8px] font-medium"
        >
      ...
      <div className="flex-1 flex items-end gap-0.5 h-8">
        {(mode === 'days' ? [15, 25, 35, 42, 50, 55, 60] : [20, 40, 55, 62, 68, 72, 78]).map((h, i) => (
          <motion.div
            key={i}
            initial={false}
            animate={{ height: `${h}%` }}
...
export function LTVAnalyticsFeatures({ variant = 'bento' }: Props): React.JSX.Element {
  switch (variant) {
    case 'grid':
      return <GridFeatures />;
    case 'tabs':
