...
import { ArrowRightIcon, CheckIcon, CodeIcon, ZapIcon, ShieldIcon, RefreshCwIcon, TerminalIcon, BookOpenIcon } from 'lucide-react';
            ...
            <BlurFade delay={0.25}>
              <div className="flex flex-col sm:flex-row items-start gap-4 pt-2">
                <motion.div
                  ...
                  animate={{ y: shouldReduceMotion ? 0 : isHovered === 'docs' ? -2 : 0 }}
          ...
          <BlurFade delay={0.2}>
            ...
            >
              ...
              <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-3">
                ...
                <span className="ml-2 text-xs text-zinc-500">{CODE_SNIPPETS[platform].filename}</span>
              </div>
              {/* Code content */}
              <AnimatePresence mode="wait">
                <motion.pre
                  ...
                  <code className="text-zinc-300">
                    {CODE_SNIPPETS[platform].code.split('\n').map((line, i) => (
                      <div key={i} className="whitespace-pre">
                        ...
                        ) : line.includes('Adapty') || line.includes('adapty') ? (
                          <>
                            {line.split(/(Adapty\.\w+|adapty\.\w+|"[^"]*"|'[^']*'|\btry\b|\bawait\b|\blet\b|\bval\b|\bconst\b)/).map((part, j) => {
                              if (part.match(/^(Adapty|adapty)\./)) return <span key={j} className="text-purple-400">{part}</span>;
                              if (part.match(/^["']/)) return <span key={j} className="text-green-400">{part}</span>;
                              if (['try', 'await', 'let', 'val', 'const'].includes(part)) return <span key={j} className="text-pink-400">{part}</span>;
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          <BlurFade delay={0.15}>
            <p className="mt-6 max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg">
              Focus on building new products, leave subscription infrastructure to us. Our SDK handles all the complexity across iOS, Android, and cross-platform frameworks.
            </p>
          </BlurFade>
          ...
          <BlurFade delay={0.2}>
            <div className="flex flex-wrap justify-center gap-3 mt-8">
              {BENEFITS.map((benefit, index) => (
                <div key={index} className="flex items-center gap-2 px-4 py-2 rounded-full bg-muted/50 border border-border/50 text-sm">
                  <benefit.icon className="size-4 text-primary" />
                  {benefit.text}
                </div>
              ))}
          ...
          <BlurFade delay={0.3}>
            <motion.div
              ...
              className="mt-12 w-full max-w-2xl mx-auto overflow-hidden rounded-xl border bg-zinc-950 shadow-lg"
            >
              <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-3">
                <div className="flex gap-1.5">
                  <div className="size-3 rounded-full bg-red-500/80" />
                ...
                <span className="ml-2 text-xs text-zinc-500">Quick integration</span>
              </div>
              <pre className="overflow-x-auto p-4 text-sm leading-relaxed text-left">
                <code className="text-zinc-300">
                  <span className="text-zinc-500">// Just 3 lines to get started</span>{'\n'}
                  <span className="text-pink-400">const</span> paywall = <span className="text-pink-400">await</span> <span className="text-purple-400">adapty.getPaywall</span>(<span className="text-green-400">&apos;premium&apos;</span>);{'\n'}
                  <span className="text-pink-400">const</span> config = <span className="text-pink-400">await</span> <span className="text-purple-400">adapty.getViewConfiguration</span>(paywall);{'\n'}
                  <span className="text-pink-400">await</span> <span className="text-purple-400">adapty.makePurchase</span>(product);
                </code>
              </pre>
            </motion.div>
          </BlurFade>
...
function TerminalHero() {
  ...
  const TERMINAL_STEPS = [
    ...
    { command: '$ adapty init --key YOUR_API_KEY', output: 'Adapty initialized successfully!' },
    { command: '$ adapty show-paywall premium', output: 'Paywall "premium" loaded with 3 products' },
  ];
      ...
      <div className="container py-16 sm:py-20 md:py-24 relative z-10">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
            ...
            >
              ...
              <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-3">
                ...
                <div className="ml-2 flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-500">Terminal - adapty-quickstart</span>
                </div>
              </div>
              {/* Terminal content */}
          ...
          <div className="flex flex-col gap-6">
            ...
            <BlurFade delay={0.15}>
              <p className="max-w-xl text-left text-base leading-relaxed text-muted-foreground sm:text-lg">
                Install our SDK, initialize with your API key, and start showing paywalls. No complex setup, no backend required. Just code.
              </p>
            </BlurFade>
            ...
            <BlurFade delay={0.2}>
              <div className="space-y-3">
                {[
                  ...
                  { icon: CheckIcon, text: 'Cross-platform support (iOS, Android, RN, Flutter, Unity)' },
...
// MAIN EXPORT
// =============================================================================
export const FOR_DEVELOPERS_HERO_VARIANTS = ['split', 'centered', 'terminal'] as const;
export type ForDevelopersHeroVariant = typeof FOR_DEVELOPERS_HERO_VARIANTS[number];

type Props = {
  variant?: ForDevelopersHeroVariant;
};

export function ForDevelopersHero({ variant = 'split' }: Props): React.JSX.Element {
  switch (variant) {
    case 'centered':
      return <CenteredHero />;
    case 'terminal':
