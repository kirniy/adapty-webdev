...
const INTEGRATIONS = [
  ...
  {
    name: 'Apple Ads',
    link: '/integrations/apple-search-ads/',
    category: 'Ads'
  },
  {
    name: 'AppsFlyer',
  ...
  },
  {
    ...
    link: '/integrations/google-analytics-firebase/',
    category: 'Analytics'
  },
  {
    name: 'Google Cloud Storage',
    link: '/integrations/google-cloud-storage/',
    category: 'Cloud Storage'
  },
  ...
  {
    name: 'Pushwoosh',
    link: '/integrations/pushwoosh/',
    category: 'Engagement'
  },
  {
    name: 'SplitMetrics',
    link: '/integrations/splitmetrics/',
    category: 'Attribution'
  },
  {
    name: 'Singular',
    link: '/integrations/singular/',
    category: 'Attribution'
  },
...
// Integration card with hover animation - Linear style
  ...
  return (
    ...
    >
      <Link
        ...
        className="block h-full relative overflow-hidden rounded-xl border border-border/50 bg-background/50 p-4 text-center transition-colors duration-150 hover:border-white/20 hover:bg-muted/30 active:scale-[0.98]"
      >
        <Spotlight
          className="from-primary/20 via-primary/10 to-transparent"
          size={120}
        />
        <p className="font-medium text-sm text-muted-foreground group-hover:text-foreground transition-colors duration-150 relative z-10">
          {integration.name}
        </p>
      </Link>
    </motion.div>
...
// SDK badge with hover animation - Linear style
function SDKBadge({ sdk }: { sdk: (typeof SDKS)[0] }) {
  ...
  return (
    ...
    >
      <Link
        href={sdk.link}
        className="block px-4 py-2 rounded-lg bg-muted/50 border border-border/50 text-sm font-medium hover:border-white/20 hover:text-primary transition-colors duration-150 ease-out active:scale-[0.98]"
      >
        {sdk.name}
      </Link>
    </motion.div>
...
// Event type badge with hover animation - Linear style
function EventBadge({ event }: { event: string }) {
  ...
  return (
    <motion.span
      whileHover={shouldReduceMotion ? undefined : { y: -2 }}
      transition={{ duration: 0.15, ease: 'easeOut' }}
      className="inline-block px-4 py-2 rounded-full bg-muted/50 border border-border/50 text-sm cursor-default hover:border-white/20 transition-colors duration-150"
    >
      {event}
    </motion.span>
  );
  ...
  return (
    <GridSection className="relative overflow-hidden">
      ...
      <div className="container py-20 relative z-10">
        ...
        <BlurFade delay={0.4}>
          <div className="mt-20">
            <SiteHeading title="Send subscription events when a user" />
          ...
          <div className="mt-20 text-center">
            <div className="relative inline-block p-8 rounded-2xl bg-muted/50 border border-border/50 max-w-2xl overflow-hidden">
              ...
              <p className="text-muted-foreground mb-4 relative z-10">
                Track purchase events, verify SDK installation, check
                integrations and A/B tests - all with Adapty SDK.
              </p>
              <Link
        ...
        <BlurFade delay={0.5}>
          <div className="mt-20">
            <SiteHeading title="Get the SDK for your platform" />
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              {SDKS.map((sdk, index) => (
                <SDKBadge
                  key={index}
