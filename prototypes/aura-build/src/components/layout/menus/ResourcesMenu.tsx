...
function ResourceLink({ title, href, icon, badge }: ResourceLinkProps) {
  return (
    <Link
      href={href}
      className="flex items-center gap-2 p-2 rounded-lg hover:bg-stone-100 transition-colors group"
    >
      <Image src={icon} alt="" width={20} height={20} className="w-5 h-5" />
      <span className="text-sm text-stone-700 group-hover:text-stone-900">{title}</span>
      {badge && (
        <span className="px-1.5 py-0.5 text-[10px] font-semibold uppercase bg-brand-lime/20 text-green-700 rounded">
          {badge}
        </span>
      )}
    </Link>
...
export function ResourcesMenu() {
  return (
    <div className="w-[720px] p-6 grid grid-cols-3 gap-8">
      {/* LEARN Column */}
      <div>
        <h4 className="text-xs font-semibold text-stone-400 uppercase tracking-wider mb-3 px-2">
          Learn
        </h4>
